...
// Substitueix l'estructura NonImplementedBallTree per la teva implementacio del
// BallTree entre les linies [35,40], al final d'aquest codi font.
...
struct NonImplementedBallTree {
  void mostraMissatge() {
    std::cout << "==== SI US PLAU, INSTANCIA CORRECTAMENT LA PLANTILLA DEL "
                 "AVALUADOR ===="
              << std::endl;
  }
  ...
  Coordinate nodeMesProper(Coordinate pdi, Coordinate &Q,
                           BallTree *bola_arrel) {
    mostraMissatge();
...
template <class T> class AvaluadorBallTree {
private:
  T *m_tree;

public:
  AvaluadorBallTree() { m_tree = new T(); }
  ...
  BallTree *cridaGetArrel() { return m_tree->getArrel(); }
...
// Substitueix del seguent template l'estructura NonImplementedBallTree per la
// teva classe on tens implementat el BallTree amb els metodes preOrdre,
// inOrdre, postOrdre, nodeMesProper, construirArbre i getArrel.
